...
def add_well_profile_app():
    st.markdown("<h2 style='text-align: center; color: red;'>WELLBORE TRAGECTORY PLOTTER</h2>", unsafe_allow_html=True)
    ...
    building_preference = st.selectbox(
            ...
            ('Load existing trajectory', 'Manual trajectory', 'Create shape-based trajectory', 'Create trajectory from 2 points')
        )
    ...
    if building_preference == 'Manual trajectory':
        ...
        for x in range(wells_no):
            ...
            with c1:
                well_name = st.text_input('Set name:', value='well ' + str(x+1))
            with v1:
                well_name_val1 = st.text_input('MD :', value='', key='well_name_val1'+str(x))
            with v2:
                well_name_val2 = st.text_input('Inclination :', value='', key='well_name_val2'+str(x))
            with v3:
                well_name_val3 = st.text_input('Azimuth:', value='', key='well_name_val3'+str(x))
            with v4:
                well_name_val4 = st.text_input('Tvd:', value='', key='well_name_val4'+str(x))
        ...
        if len(wellbores_data) == 0:
            ...
            if st.button("Plot Well", type="primary"):
                fig = wellbores_data[0].plot(plot_type=plot_type,
                                             x_axis=xaxis,
                                             y_axis=yaxis,
                                             add_well=wellbores_data[1:],
...
def data_and_plot(trajectory, show_data, dark, color):
    ...
    style = {'darkMode': dark, 'color': color_data[color], 'size': marker_size, 'height': 900 }
